#!/usr/bin/env python3
"""post_industrial.py — finish the INDUSTRIAL trailer: burn captions + cards.

Same pipeline as post_jazz.py: captions hold until the next one starts
(clamped), text goes through textfile= to dodge ffmpeg drawtext's
multibyte-glyph width bug, light limiter on the body audio.

Reads /tmp/rec_start.txt + /tmp/marks.txt (see lib.mark / record.sh).
Output: $OUT (default ~/reaclaw_industrial_trailer.mp4).
"""
import os
import subprocess

RAW = os.environ.get("RAW", "/tmp/raw_industrial.mp4")
OUT = os.environ.get("OUT", os.path.expanduser("~/reaclaw_industrial_trailer.mp4"))
WORK = "/tmp/trailer_industrial_build"
...
def make_card(path, line1, line2="", seconds=3.0):
    ...
    sh("ffmpeg", "-y", "-hide_banner",
       "-f", "lavfi", "-i", f"color=c=0x100a0a:s={SIZE}:d={seconds}",
...
def main():
    ...
    title = make_card(f"{WORK}/title.mp4", "ReaClaw",
                      "INDUSTRIAL -- a late-80s banger, built live by an AI over HTTPS", 4.0)
