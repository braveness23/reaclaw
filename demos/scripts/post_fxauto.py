#!/usr/bin/env python3
"""post_fxauto.py — finish the FX & AUTOMATION trailer: burn captions + cards.

Same pipeline as post_zoom.py (the newest/cleanest caption burner): each
...
Output: $OUT (default ~/reaclaw_fxauto_trailer.mp4).
"""
import os
import subprocess

RAW = os.environ.get("RAW", "/tmp/raw_fxauto.mp4")
OUT = os.environ.get("OUT", os.path.expanduser("~/reaclaw_fxauto_trailer.mp4"))
WORK = "/tmp/trailer_fxauto_build"
...
def main():
    ...
    title = make_card(f"{WORK}/title.mp4", "ReaClaw",
                      "FX & Automation — one API, every plugin", 4.0)
