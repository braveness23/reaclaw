#!/usr/bin/env python3
"""post_zoom.py — finish the ZOOM trailer: burn captions + zoom-themed cards.

Same pipeline as post.py, with two refinements:
  * each caption holds until the NEXT caption starts (clamped), so the lower
    third is continuous rather than blinking on a fixed timer;
  * zoom-themed title/end cards.

Reads /tmp/rec_start.txt + /tmp/marks.txt (see lib.mark / record.sh).
Output: $OUT (default ~/reaclaw_zoom_trailer.mp4).
"""
import os
import subprocess

RAW = os.environ.get("RAW", "/tmp/raw_zoom.mp4")
OUT = os.environ.get("OUT", os.path.expanduser("~/reaclaw_zoom_trailer.mp4"))
WORK = "/tmp/trailer_zoom_build"
    ...
    with open("/tmp/marks.txt") as f:
        for line in f:
            ...
            if t >= 0:
                raw.append((t, label))
    raw.sort()
    # hold each caption until the next one (clamped to [MIN_HOLD, MAX_HOLD])
    marks = []
...
def fold(text):
    """ASCII-fold multibyte punctuation. ffmpeg's drawtext UNDERCOUNTS text_w
    for multibyte glyphs (em dash, curly quotes, ellipsis), which throws off
    x=(w-text_w)/2 centering and runs the tail of the line off the right edge.
    Text reaches drawtext via textfile= (below) so no filter-escaping is needed
    — only the multibyte glyphs have to go."""
    for uni, asc in (("—", "-"), ("–", "-"), ("’", "'"), ("‘", "'"),
                     ("“", '"'), ("”", '"'), ("…", "...")):
...
def caption_filter(marks):
    # Supply each caption via textfile= rather than text='...': apostrophes and
    # colons in the copy otherwise need fragile filtergraph quote-escaping that
    # does not survive being passed as a single argv. A file sidesteps all of it.
    draws = []
...
def main():
    ...
    title = make_card(f"{WORK}/title.mp4", "ReaClaw",
                      "Zoom — drive REAPER’s camera with code", 4.0)
