#!/usr/bin/env python3
"""post_postpunk.py — finish the POST-PUNK trailer: burn captions + cards.

Same pipeline as post_industrial.py: captions hold until the next one starts
(clamped), text goes through textfile= to dodge ffmpeg drawtext's
multibyte-glyph width bug, light limiter on the body audio. Additionally cuts
the dead air after the END mark (the recorder keeps rolling until it's killed
from another shell).

Reads /tmp/rec_start.txt + /tmp/marks.txt (see lib.mark / record.sh).
Output: $OUT (default ~/reaclaw_postpunk_trailer.mp4).
"""
import os
import subprocess

RAW = os.environ.get("RAW", "/tmp/raw_postpunk.mp4")
OUT = os.environ.get("OUT", os.path.expanduser("~/reaclaw_postpunk_trailer.mp4"))
WORK = "/tmp/trailer_postpunk_build"
FONT = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"

TRIM = float(os.environ.get("TRIM", "0"))
TAIL = 1.5          # seconds kept after the END mark
MIN_HOLD = 2.0
MAX_HOLD = 7.0
...
def make_card(path, line1, line2="", seconds=3.0):
    ...
    sh("ffmpeg", "-y", "-hide_banner",
       "-f", "lavfi", "-i", f"color=c=0x0a0d14:s={SIZE}:d={seconds}",
...
def main():
    ...
    marks, end_t = load_marks()
    print(f"{len(marks)} captions, body ends at {end_t}")

    title = make_card(f"{WORK}/title.mp4", "ReaClaw",
                      "POST-PUNK -- drums that make music, built live by an AI over HTTPS", 4.0)
    body = make_body(marks, end_t)
