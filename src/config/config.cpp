...
json default_config() {
    return {{"server", {{"host", "0.0.0.0"}, {"port", 9091}, {"thread_pool_size", 4}}},
            {"tls",
             {{"enabled", true},
...
bool Config::load(Config& cfg, const std::string& resource_path) {
    ...
    if (cfg.db_path.empty())
        cfg.db_path = cfg.resource_dir + "reaclawdb.sqlite";
...
bool Config::save() const {
    json j = {{"server", {{"host", host}, {"port", port}, {"thread_pool_size", thread_pool_size}}},
              {"tls",
               {{"enabled", tls_enabled},
                {"generate_if_missing", tls_generate_if_missing},
