...
void handle_scripts_register(const httplib::Request& req, httplib::Response& res) {
    ...
    if (!result.registered && result.syntax_error_line > 0) {
        json_ok(res,
                {{"registered", false},
                 {"syntax_error",
                  {{"line", result.syntax_error_line}, {"message", result.syntax_error_message}}}});
        return;
    }
    ...
    if (!result.registered && !result.syntax_error_message.empty()) {
        json_ok(res,
                {{"registered", false},
                 {"syntax_error", {{"line", 0}, {"message", result.syntax_error_message}}}});
        return;
    }
...
void handle_scripts_cache(const httplib::Request& req, httplib::Response& res) {
    std::string tag_filter = req.has_param("tags") ? req.get_param_value("tags") : "";
    ...
    for (auto& r : rows) {
        nlohmann::json entry = {{"action_id", r.at("id")},
                                {"name", r.at("name")},
                                {"execution_count", 0},
                                {"created_at", r.at("created_at")}};

        try {
            entry["tags"] = nlohmann::json::parse(r.at("tags"));
        } catch (...) {
            entry["tags"] = nlohmann::json::array();
        }

        try {
            entry["execution_count"] = std::stoi(r.at("execution_count"));
        } catch (...) {
        }

        const std::string& last = r.at("last_executed");
        if (!last.empty())
            entry["last_executed"] = last;
