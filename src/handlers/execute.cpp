...
nlohmann::json build_feedback() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (auto* v = static_cast<double*>(GetSetMediaTrackInfo(t, "D_VOL", nullptr)))
            vol = *v;
        if (auto* p = static_cast<double*>(GetSetMediaTrackInfo(t, "D_PAN", nullptr)))
            pan = *p;
        bool muted = false;
        if (auto* m = static_cast<bool*>(GetSetMediaTrackInfo(t, "B_MUTE", nullptr)))
            muted = *m;
        tracks.push_back({{"index", i},
                          {"name", name},
                          {"muted", muted},
...
void handle_execute_sequence(const httplib::Request& req, httplib::Response& res) {
    ...
    for (int i = 0; i < n_steps; i++) {
        ...
        std::string step_id = step.contains("id") ? (step["id"].is_number()
                                                             ? std::to_string(step["id"].get<int>())
