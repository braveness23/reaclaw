...
nlohmann::json track_to_json(MediaTrack* track, int index) {
    ...
    if (auto* v = static_cast<double*>(GetSetMediaTrackInfo(track, "D_VOL", nullptr))) vol = *v;
    if (auto* p = static_cast<double*>(GetSetMediaTrackInfo(track, "D_PAN", nullptr))) pan = *p;

    bool muted  = false;
    bool soloed = false;
    bool armed  = false;
    if (auto* m = static_cast<bool*>(GetSetMediaTrackInfo(track, "B_MUTE",   nullptr))) muted  = *m;
    if (auto* s = static_cast<int*> (GetSetMediaTrackInfo(track, "I_SOLO",   nullptr))) soloed = *s != 0;
    if (auto* a = static_cast<int*> (GetSetMediaTrackInfo(track, "I_RECARM", nullptr))) armed  = *a != 0;
...
nlohmann::json read_project_state() {
...
#ifdef _WIN32
    {
        ...
        if (sep2 != std::string::npos &&
            (sep == std::string::npos || sep2 > sep)) sep = sep2;
    }
#endif
...
void handle_state_set_track(const httplib::Request& req, httplib::Response& res) {
    int index = 0;
    try { index = std::stoi(req.path_params.at("index")); } catch (...) {
    ...
    auto result = Executor::post([index, body]() -> nlohmann::json {
        int n = CountTracks(nullptr);
        if (index < 0 || index >= n) return {{"_not_found", true}};
        MediaTrack* track = GetTrack(nullptr, index);
        if (!track) return {{"_not_found", true}};
    ...
    if (result.contains("_timeout"))   { json_error(res, 408, "Main thread timeout", "TIMEOUT"); return; }
...
void handle_state_items(const httplib::Request& req, httplib::Response& res) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (take) {
            if (const char* tn = GetTakeName(take)) take_name = tn;
        }
...
void handle_state_selection(const httplib::Request& req, httplib::Response& res) {
    ...
    for (int i = 0; i < n_items; i++) {
        ...
        sel_items.push_back({{"index", i}, {"position", pos}, {"length", len}, {"track_index", tidx}});
    }
