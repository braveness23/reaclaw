...
void register_routes(httplib::SSLServer& svr, const Config& cfg) {
    ...
    svr.Get(R"(/catalog/(\d+))",   auth_wrap(cfg, [](const httplib::Request& req, httplib::Response& res) {
    ...
    svr.Post(R"(/state/tracks/(\d+))", auth_wrap(cfg, [](const httplib::Request& req, httplib::Response& res) {
    ...
    svr.Get(R"(/scripts/([^/]+))",     auth_wrap(cfg, [](const httplib::Request& req, httplib::Response& res) {
        const_cast<httplib::Request&>(req).path_params["id"] = req.matches[1];
        Handlers::handle_scripts_get(req, res);
    }));
    svr.Delete(R"(/scripts/([^/]+))",  auth_wrap(cfg, [](const httplib::Request& req, httplib::Response& res) {
        const_cast<httplib::Request&>(req).path_params["id"] = req.matches[1];
    ...
    svr.set_error_handler([](const httplib::Request&, httplib::Response& res) {
        if (res.status == 404) {
            res.set_content(
                R"({"error":"Not found","code":"NOT_FOUND","context":{}})",
                "application/json");
        }
    });
}
