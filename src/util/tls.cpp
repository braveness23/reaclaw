...
bool generate_self_signed(const std::string& cert_path,
    ...
    if (!kctx) { Log::error("TLS: EVP_PKEY_CTX_new_id failed"); goto done; }
    if (EVP_PKEY_keygen_init(kctx) <= 0) { Log::error("TLS: keygen_init failed"); goto done; }
    if (EVP_PKEY_CTX_set_rsa_keygen_bits(kctx, 4096) <= 0) {
        Log::error("TLS: set_rsa_keygen_bits failed");
        goto done;
    }
    if (EVP_PKEY_keygen(kctx, &pkey) <= 0) { Log::error("TLS: keygen failed"); goto done; }

    x509 = X509_new();
    if (!x509) { Log::error("TLS: X509_new failed"); goto done; }
    ...
    if (!f) { Log::error("TLS: cannot open for writing: " + cert_path); goto done; }
    ...
    if (!f) { Log::error("TLS: cannot open for writing: " + key_path); goto done; }
