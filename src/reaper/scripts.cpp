...
SyntaxResult parse_luac_output(const std::string& output) {
    ...
    res.message = output;
    while (!res.message.empty() && (res.message.back() == '\n' || res.message.back() == '\r'))
        res.message.pop_back();
    return res;
}
...
SyntaxResult check_lua_syntax(const std::string& body) {
    ...
    // Write body to temp file
    {
        ...
        if (!f)
            return {true, 0, ""};  // Can't write temp — skip check
        f << body;
    }
...
RegisterResult register_script(const std::string& name,
    ...
    {
        auto existing = g_db.query("SELECT id, script_path FROM scripts WHERE name = ?1", {name});
        if (!existing.empty()) {
    ...
    if (g_config.validate_syntax) {
        ...
        if (!syn.ok) {
            ...
            Log::warn("Scripts: syntax error in '" + name + "' line " + std::to_string(syn.line) +
                      ": " + syn.message);
            return result;
        }
    }
    ...
    // --- Path traversal guard: verify path stays inside scripts_dir ---
    {
        ...
        if (rel.empty() || rel.begin()->string() == "..") {
            result.internal_error =
                    "Script path escapes scripts directory (path traversal rejected)";
    ...
    std::string wrapped =
            "local _rc_ok, _rc_err = pcall(function()\n" + body +
            "\nend)\n"
            "if not _rc_ok then\n"
    ...
    g_db.query(
            ...
            "VALUES(?1, ?2, ?3, ?4, ?5, ?6)",
            {action_id, name, body, script_path, tags_json.dump(), std::to_string(reaper_cmd)});
...
bool unregister_script(const std::string& action_id) {
    auto rows = g_db.query("SELECT script_path FROM scripts WHERE id = ?1", {action_id});
    if (rows.empty())
        return false;
    ...
    fs::remove(script_path, ec);
    if (ec)
        Log::warn("Scripts: could not delete " + script_path + ": " + ec.message());
